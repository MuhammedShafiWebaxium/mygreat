...
import { ArrowRight, Check, Compass, GitCompareArrows, MapPin, Sparkles, Trophy, X } from 'lucide-react'
...
export default function UniversitiesTab({ student, shortlisted, recommendations, onShortlistToggle }: Props) {
  ...
  const universities = useMemo<ComparisonUniversity[]>(() => {
    ...
    const rows: ComparisonUniversity[] = shortlisted.map((item) => ({
      ...
      acceptance: item.acceptance || 'Not published', knownFor: item.knownFor || 'Not published', course: student.target || 'Selected course', shortlisted: true, university: item,
    }))
    recommendations.forEach((item) => {
      if (!rows.some((row) => row.id === item.id)) rows.push({
        id: item.id, name: item.name, city: item.city, rank: item.rank, tuition: item.tuition,
        acceptance: item.acceptance, knownFor: item.knownFor || student.target, course: student.target || 'Selected course', shortlisted: shortlistedIds.has(item.id),
...
function UniversityCard({ university, selected, disabled, saving, onToggle, onShortlist, recommended = false }: { university: ComparisonUniversity; selected: boolean; disabled: boolean; saving: boolean; onToggle: () => void; onShortlist: () => void; recommended?: boolean }) {
  return <article className={cn('flex flex-col rounded-2xl border p-4.5 transition-all', selected ? 'border-amber-400/50 bg-amber-400/[.07] ring-1 ring-amber-400/20' : 'border-white/[0.07] bg-white/[0.02] hover:border-amber-400/30')}>
    ...
    <div className="mt-4 flex-1 rounded-xl border border-white/[0.06] bg-white/[0.025] p-3"><p className="text-[9px] font-bold uppercase tracking-[.14em] text-white/30">Selected course</p><p className="mt-1.5 line-clamp-2 text-[12px] font-semibold leading-5 text-white/75">{university.course}</p>{university.knownFor!==university.course&&university.knownFor!=='Not published'&&<p className="mt-1 line-clamp-1 text-[10px] text-white/38">Known for: {university.knownFor}</p>}</div>
    <div className="mt-3 grid grid-cols-2 gap-2"><div className="rounded-xl bg-white/[0.025] px-3 py-2.5"><p className="text-[9px] uppercase tracking-wide text-white/28">Annual tuition</p><p className="mt-1 truncate text-[11px] font-semibold text-white/70">{university.tuition&&university.tuition!=='Not published'?university.tuition:'Contact university'}</p></div><div className="rounded-xl bg-white/[0.025] px-3 py-2.5"><p className="text-[9px] uppercase tracking-wide text-white/28">Acceptance</p><p className="mt-1 truncate text-[11px] font-semibold text-white/70">{university.acceptance||'Not published'}</p></div></div>
    <div className={cn('mt-3 grid gap-2',university.shortlisted?'grid-cols-1':'grid-cols-2')}>{!university.shortlisted&&<button type="button" disabled={saving} onClick={onShortlist} className={cn('inline-flex items-center justify-center gap-1.5 rounded-xl bg-amber-400 px-3 py-2.5 text-[11px] font-bold text-slate-950 transition hover:bg-amber-300',saving&&'opacity-40')}>{saving?'Saving…':'Shortlist'}</button>}<button type="button" disabled={disabled} onClick={onToggle} className={cn('inline-flex items-center justify-center gap-1.5 rounded-xl border px-3 py-2.5 text-[11px] font-semibold transition', selected ? 'border-amber-400 bg-amber-400/10 text-amber-300' : 'border-white/10 text-white/65 hover:border-amber-400/40 hover:text-amber-300', disabled && 'cursor-not-allowed opacity-35')}><span className={cn('grid size-4 place-items-center rounded border', selected ? 'border-amber-400/50' : 'border-white/20')}>{selected && <Check className="size-3" />}</span>{selected ? 'Comparing' : 'Compare'}</button></div>
  </article>
}
...
function ComparisonModal({ universities, onClose, onRemove }: { universities: ComparisonUniversity[]; onClose: () => void; onRemove: (id: string) => void }) {
  const rows: Array<[string, (item: ComparisonUniversity) => string]> = [['Course', (item) => item.course], ['Location', (item) => item.city], ['World ranking', (item) => item.rank ? `QS #${item.rank}` : 'Not published'], ['Tuition', (item) => item.tuition], ['Acceptance rate', (item) => item.acceptance], ['Known for', (item) => item.knownFor], ['Shortlisted', (item) => item.shortlisted ? 'Yes' : 'No']]
