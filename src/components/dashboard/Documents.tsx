...
import { Check, Clock, Download, FileText, LoaderCircle, ShieldCheck, Upload } from 'lucide-react'
  ...
  return <div className="document-center max-w-5xl space-y-5">
    ...
    {allSubmittedForReview && !allVerified && <motion.section variants={fadeUp} initial="hidden" animate="show" role="status" className="overflow-hidden rounded-3xl border border-amber-400/30 bg-gradient-to-br from-amber-400/[.13] via-amber-300/[.06] to-transparent p-5 shadow-[0_18px_45px_-28px_rgba(245,158,11,.8)] sm:p-6">
      <div className="flex items-start gap-4">
        <span className="relative grid size-12 shrink-0 place-items-center rounded-2xl border border-amber-400/30 bg-amber-400/15 text-amber-300">
          <LoaderCircle className="size-6 animate-spin"/>
          <span className="absolute -right-1 -top-1 size-3 rounded-full border-2 border-[#10172a] bg-amber-400"/>
        </span>
        <div className="min-w-0 flex-1">
          ...
          <div className="mt-4 flex items-center gap-2 text-[10px] font-semibold sm:gap-3">
            ...
            <span className="rounded-full border border-amber-400/30 bg-amber-400/10 px-3 py-1.5 text-amber-300">Staff review</span>
            <span className="h-px w-5 bg-white/10"/>
            <span className="rounded-full border border-white/10 px-3 py-1.5 text-white/35">Ready to apply</span>
          </div>
        </div>
      </div>
    </motion.section>}
    ...
    <Panel>
      <PanelTitle>Application prerequisites</PanelTitle>
      <div className="grid gap-3 px-4 pb-5 sm:grid-cols-2">{REQUIRED_DOCUMENTS.map(required => {
        const document = documents.find(item => item.name === required.name)
        const isVerified = document?.status === 'VERIFIED'
        const isPending = document?.status === 'PENDING'
        return <article key={required.id} className="rounded-2xl border border-white/[.08] p-4">
          <div className="flex gap-3"><div className={`grid size-10 place-items-center rounded-xl border ${isVerified ? 'border-emerald-400/25 bg-emerald-400/10 text-emerald-300' : isPending ? 'border-amber-400/25 bg-amber-400/10 text-amber-300' : 'border-dashed border-white/15 text-white/35'}`}>{isVerified ? <Check className="size-4"/> : isPending ? <Clock className="size-4"/> : <FileText className="size-4"/>}</div><div className="min-w-0 flex-1"><p className="text-sm font-semibold">{required.name}</p><p className="mt-1 text-[10px] text-white/40">{isVerified ? 'Verified' : isPending ? 'Uploaded · under verification' : document?.note || 'Upload required'}</p></div></div>
          <div className="mt-4 flex gap-2"><button disabled={uploading} onClick={() => choose(required.id)} className="action flex-1 justify-center"><Upload className="size-3.5"/>{document?.storageKey ? 'Replace' : 'Upload'}</button>{document?.storageKey && <a href={`/api/workflow?documentId=${document.id}`} className="icon" title="Download"><Download className="size-4"/></a>}</div>
        </article>
      })}</div>
    </Panel>
