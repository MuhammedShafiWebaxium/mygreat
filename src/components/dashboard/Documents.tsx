...
type RequiredDocument = { id:string;name:string;accept:string;active:boolean;sortOrder:number }

export default function Documents({ documents, requiredDocuments, onChanged }: { documents: ServerDocument[]; requiredDocuments:RequiredDocument[]; onChanged: () => Promise<unknown> }) {
  const [target, setTarget] = useState<string>(requiredDocuments[0]?.id??'')
  const [error, setError] = useState('')
  const [uploadingId, setUploadingId] = useState<string | null>(null)
  const input = useRef<HTMLInputElement>(null)
  const uploaded = requiredDocuments.filter((required) => documents.some((document) => document.name === required.name && document.storageKey)).length
  const verified = requiredDocuments.filter((required) => documents.some((document) => document.name === required.name && document.status === 'VERIFIED')).length
  const allUploaded = requiredDocuments.length>0&&uploaded === requiredDocuments.length
  const allVerified = requiredDocuments.length>0&&verified === requiredDocuments.length
  const allSubmittedForReview = allUploaded && requiredDocuments.every((required) => documents.some((document) => document.name === required.name && ['PENDING', 'VERIFIED'].includes(document.status)))
  const verificationProgress = requiredDocuments.length?verified / requiredDocuments.length * 100:0
  ...
  return <div className="document-center max-w-5xl space-y-5">
    ...
    <Panel className="flex items-center gap-4 p-5"><ShieldCheck className="size-6 text-amber-300"/><div className="flex-1"><div className="flex justify-between text-xs"><span>{uploaded}/{requiredDocuments.length} uploaded · {verified}/{requiredDocuments.length} verified</span><span>{Math.round(verificationProgress)}% verified</span></div><Bar value={verificationProgress} className="mt-2"/></div></Panel>
    {error && <p role="alert" className="rounded-xl border border-rose-400/25 bg-rose-400/10 p-3 text-xs text-rose-300">{error}</p>}
    <Panel><PanelTitle>Application prerequisites</PanelTitle><div className="grid gap-3 px-4 pb-5 sm:grid-cols-2">{requiredDocuments.map((required) => {
    ...
    <input ref={input} type="file" hidden accept={requiredDocuments.find((item) => item.id === target)?.accept} onChange={(event) => upload(event.target.files?.[0])}/>
  </div>
}
