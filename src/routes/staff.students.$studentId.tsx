...
function StudentDetail() {
  ...
  return <div className="space-y-5">
    ...
    <section className="grid gap-5 xl:grid-cols-2"><div className="staff-card rounded-3xl border border-white/[.07] bg-white/[.025] p-6"><div className="flex items-center gap-2"><GraduationCap className="size-4 text-amber-300"/><h3 className="font-display text-xl">Academic profile</h3></div><dl className="mt-5 grid grid-cols-2 gap-5 text-xs">{[['Destination',country?.name||'Not selected'],['Education',student.profile?.educationLevel||'Not provided'],['Degree',student.profile?.degree||'Not provided'],['Course',student.profile?.field||'Not provided'],['GPA',student.profile?.gpa?String(student.profile.gpa):'Not provided'],['Intake',student.profile?.preferredIntake||'Not provided']].map(([label,value])=><div key={label}><dt className="text-[9px] uppercase tracking-[.15em] text-white/30">{label}</dt><dd className="mt-1.5 text-white/75">{value}</dd></div>)}</dl></div><DesiredUniversities studentId={student.id} universities={student.shortlist.map(({university})=>university)} appliedUniversityIds={[...new Set(student.applications.map(application=>application.universityId))]} selectedCourse={student.profile?.field??''}/></section>
