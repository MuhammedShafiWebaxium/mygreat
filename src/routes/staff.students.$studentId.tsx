...
function RequiredDocuments({
  ...
  const verified = required.filter(
    (item) => item.document?.status === 'VERIFIED',
  ).length
  ...
  return (
    <section className="staff-card overflow-hidden rounded-3xl border border-white/[.07] bg-white/[.025]">
      <header className="border-b border-white/[.07] p-5 sm:p-6">
        <div className="flex flex-col gap-5 sm:flex-row sm:items-center sm:justify-between">
          <div>
            ...
            <h3 className="mt-1.5 font-display text-2xl">Required documents</h3>
            <p className="mt-1 text-xs text-white/38">
              Review each uploaded file before this student can submit an
              application.
            </p>
          </div>
        ...
        <div className="mt-5">
          <div className="mb-2 flex justify-between text-[10px]">
            <span className="text-white/38">Verification progress</span>
        ...
        {pending > 0 && (
          <div className="mt-4 flex items-center gap-2 rounded-xl border border-amber-400/20 bg-amber-400/[.07] px-3 py-2.5 text-[11px] text-amber-200">
            <Clock3 className="size-3.5" />
            <strong>
              {pending} {pending === 1 ? 'document is' : 'documents are'}{' '}
      ...
      <div className="grid gap-3 p-4 sm:p-5 lg:grid-cols-2">
        {required.map(({ id, name, document }, index) => {
          const status = document?.status ?? 'MISSING'
          const verified = status === 'VERIFIED',
            pending = status === 'PENDING',
            needsReplacement = status === 'NEEDED' || status === 'REJECTED'
          const Icon = verified
            ? CheckCircle2
            : pending
              ? Clock3
          ...
          return (
            <article
              key={id}
              className={cn(
                'relative overflow-hidden rounded-2xl border p-4 transition sm:p-5',
                verified
                  ? 'border-emerald-400/20 bg-emerald-400/[.035]'
                  : pending
                    ? 'border-amber-400/25 bg-amber-400/[.045]'
                    : needsReplacement
                      ? 'border-rose-400/20 bg-rose-400/[.035]'
                      : 'border-dashed border-white/10 bg-white/[.015]',
              )}
            >
                    ...
                    verified
                      ...
                      : pending
                        ...
                        : needsReplacement
                          ...
                          : 'border-white/10 bg-white/[.035] text-white/30',
                  )}
                >
                ...
                <div className="min-w-0 flex-1">
                  <div className="flex items-start justify-between gap-3">
                    <div>
                      ...
                      <h4 className="mt-1 text-sm font-semibold">{name}</h4>
                    </div>
                    <span
                      className={cn(
                        'shrink-0 rounded-full border px-2.5 py-1 text-[9px] font-bold uppercase tracking-wide',
                        verified
                          ? 'border-emerald-400/25 bg-emerald-400/10 text-emerald-300'
                          : pending
                            ? 'border-amber-400/25 bg-amber-400/10 text-amber-300'
                  ...
                  <p className="mt-2 min-h-4 text-[10.5px] text-white/38">
                    {document?.note ||
                      (document
                        ? 'Uploaded by student.'
                        : 'Waiting for the student to upload this file.')}
                  </p>
                </div>
              </div>
              {document?.storageKey && (
                <div className="mt-4 flex flex-col gap-2 border-t border-white/[.06] pt-4 sm:flex-row">
                  <a
                    href={`/api/workflow?documentId=${document.id}`}
                    target="_blank"
                    rel="noreferrer"
                    className="action flex flex-1 items-center justify-center gap-2"
                  >
                  ...
                  {pending && (
                    <>
                      <button
                        ...
                        onClick={() => review(document.id, 'VERIFIED')}
                        className="inline-flex items-center justify-center gap-1.5 rounded-xl bg-emerald-400 px-4 py-2.5 text-[11px] font-bold text-[#07140f] transition hover:bg-emerald-300 disabled:opacity-50"
                      >
                      ...
                      <button
                        disabled={reviewingId === document.id}
                        onClick={() => review(document.id, 'NEEDED')}
                        className="inline-flex items-center justify-center gap-1.5 rounded-xl border border-rose-400/25 px-4 py-2.5 text-[11px] font-semibold text-rose-300 transition hover:bg-rose-400/10 disabled:opacity-50"
                      >
                  ...
                  {verified && (
                    <button
                      disabled={reviewingId === document.id}
                      onClick={() => review(document.id, 'NEEDED')}
                      className="inline-flex items-center justify-center gap-1.5 rounded-xl border border-white/10 px-4 py-2.5 text-[11px] font-semibold text-white/48 transition hover:border-rose-400/25 hover:bg-rose-400/[.07] hover:text-rose-300 disabled:opacity-50"
                    >
                      <RefreshCw className="size-3.5" />
                      Request replacement
                    </button>
...
function StudentDetail() {
  ...
  return (
    <div className="space-y-5">
      <Link
        href="/staff/students"
        className="inline-flex items-center gap-2 text-xs text-white/45 hover:text-amber-300"
      >
      ...
      <section className="grid gap-5 xl:grid-cols-2">
        <div className="staff-card rounded-3xl border border-white/[.07] bg-white/[.025] p-6">
          <div className="flex items-center gap-2">
            <GraduationCap className="size-4 text-amber-300" />
            <h3 className="font-display text-xl">Academic profile</h3>
          </div>
          <dl className="mt-5 grid grid-cols-2 gap-5 text-xs">
            {[
              ['Destination', country?.name || 'Not selected'],
              ['Education', student.profile?.educationLevel || 'Not provided'],
              ['Degree', student.profile?.degree || 'Not provided'],
              ['Course', student.profile?.field || 'Not provided'],
              [
                'GPA',
                student.profile?.gpa
                  ? String(student.profile.gpa)
                  : 'Not provided',
              ],
              ['Intake', student.profile?.preferredIntake || 'Not provided'],
            ].map(([label, value]) => (
              <div key={label}>
                <dt className="text-[9px] uppercase tracking-[.15em] text-white/30">
                  {label}
                </dt>
                <dd className="mt-1.5 text-white/75">{value}</dd>
              </div>
            ))}
          </dl>
        </div>
        <DesiredUniversities
          ...
          universities={student.shortlist.map(({ university }) => university)}
            ...
            <div
              ...
              className={cn(
                ...
                application.isPriority
                  ? 'border-amber-300/30 bg-amber-300/[.05]'
                  : 'border-white/[.07]',
              )}
            >
              <div className="flex items-start gap-2">
                <Link
                  ...
                  className="flex-1 text-sm font-semibold transition hover:text-amber-300"
                >
                  {application.university.name}
                </Link>
                {application.isPriority ? (
                  <span className="inline-flex items-center gap-1 rounded-full bg-amber-400 px-2 py-1 text-[8px] font-bold text-slate-950">
                    <Flag className="size-2.5 fill-current" />
                    PRIORITY
                  </span>
                ) : (
                  <button
                    disabled={priorityBusy === application.id}
                    onClick={() => selectPriority(application.id)}
                    className="inline-flex items-center gap-1 rounded-lg border border-amber-300/25 px-2.5 py-1.5 text-[8px] font-bold text-amber-300 disabled:opacity-40"
                  >
                    <Flag className="size-3" />
                    {priorityBusy === application.id
                      ? 'Saving…'
                      : 'Make priority'}
                  </button>
                )}
              ...
              <div className="mt-3 flex items-center justify-between">
                <p className="text-[10px] text-sky-300">
                        {application.applicationStage.replaceAll('_', ' ')} ·{' '}
                  {application.progress}%
                </p>
                <div className="flex gap-3">
                  <Link
                    href={`/staff/applications/${application.id}`}
                    className="text-[9px] font-semibold text-amber-300"
                  >
                    Open case →
                  </Link>
                  {application.applicationStage === 'MOVE_TO_VISA' && (
                    <Link
                      href={`/staff/visas/${application.id}`}
